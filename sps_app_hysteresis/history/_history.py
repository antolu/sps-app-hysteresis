...
class PredictionHistory(QtCore.QObject):
    ...
    def add_cycle(self, cycle_data: CycleData) -> None:
        ...
        """
        with self._lock:
            log.debug(f"Adding cycle {cycle_data.cycle} to history")
            if cycle_data.cycle not in self._history:
                log.debug(f"Creating new CycleListModel for cycle {cycle_data.cycle}")
    ...
    def update_cycle(self, cycle_data: CycleData) -> None:
        ...
        """
        with self._lock:
            log.debug(f"Updating cycle {cycle_data.cycle} in history")
            if cycle_data.cycle in self._history:
                ...
                log.debug(
                    f"[{cycle_data}] not found in history, no prediction to update"
                )
            elif cycle_data.field_meas is None:  # no measurement
                log.debug(
                    f"[{cycle_data}] not found in history, no measurement to update"
                )
            else:
                log.error(f"[{cycle_data}] not found in history, cannot update")
    ...
    def model(self, cycle: str) -> CycleListModel:
        ...
        with self._lock:
            if cycle not in self._history:
                log.debug(
                    f"Creating new CycleListModel for cycle {cycle} in model() method"
                )
    ...
    def onReferenceChanged(self, cycle_data: CycleData) -> None:
        with self._lock:
            if cycle_data.cycle not in self._history:
                log.debug(
                    f"Creating new CycleListModel for cycle {cycle_data.cycle} in onReferenceChanged"
                )
                self._history[cycle_data.cycle] = CycleListModel(parent=self)
